import { useEffectAsync, useMemoAsync } from '.'
...
export const useWhisper: UseWhisperHook = (config) => {
  ...
  const onDataAvailable = async (data: Blob) => {
    ...
    try {
        if (streaming && recorder.current && encoder.current) {
            onDataAvailableCallback?.(data);

            const chunkSize = 1024 * 16; // Set your desired chunk size
            
            const buffer = await data.arrayBuffer();
            // console.log(buffer.byteLength,"buffer byte length");
            ...
            if (totalChunks > 10)
            {
              ...
              for (let i = 10; i < totalChunks; i++) {
                ...
                const mp3chunk = encoder.current.encodeBuffer(new Int16Array(chunkBuffer));
